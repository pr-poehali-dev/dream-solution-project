...
export default function Section({ id, title, subtitle, slogan, content, isActive, showButton, buttonText, services, badges }: SectionProps) {
  return (
    <section id={id} className="relative h-screen w-full snap-start flex flex-col justify-center p-8 md:p-16 lg:p-24 pt-20">
      ...
      <motion.h2
        className="font-bold leading-[1.05] tracking-tight max-w-4xl text-white"
      ...
      {services && (
        ...
        >
          {services.map((service, i) => (
            <motion.div
              key={service.label}
              className="flex flex-col items-start gap-3 p-4 border border-[#1e2a3a] rounded-lg hover:border-[#c9a84c] transition-colors bg-black/20"
              initial={{ opacity: 0, y: 20 }}
              animate={isActive ? { opacity: 1, y: 0 } : {}}
      ...
      {badges && (
        ...
        >
          {badges.map((badge, i) => (
            ...
            >
              <div className="w-12 h-12 rounded-full border border-[#c9a84c]/40 flex items-center justify-center bg-black/30">
